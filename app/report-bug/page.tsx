...
export default function ReportBugPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      ...
      page: (form.elements.namedItem('page') as HTMLInputElement).value,
      description: (form.elements.namedItem('description') as HTMLTextAreaElement).value,
      screenshotUrl: (form.elements.namedItem('screenshotUrl') as HTMLInputElement).value || undefined,
    };
        ...
        <div style={{ ...WRAP, paddingTop: 48, paddingBottom: 64 }} className="section-pad">
          <Reveal>
            ...
            <p style={{ fontSize: 15, color: C.muted, lineHeight: 1.75, maxWidth: 520 }}>
              Found something broken? Tell us about it and we'll fix it fast.
            </p>
          </Reveal>
        </div>
      </section>
            ...
            <div style={{ maxWidth: 520 }}>
              {sent ? (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <h3 style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: 20, fontWeight: 800, color: C.cream, marginBottom: 8 }}>Bug reported!</h3>
                  <p style={{ fontSize: 14, color: `${C.cream}50`, lineHeight: 1.6 }}>We'll investigate and fix it as soon as possible. Thanks for helping us improve.</p>
                </motion.div>
              ) : (
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div>
                    <label style={labelStyle}>Page where bug occurred *</label>
                    <input required type="text" name="page" placeholder="e.g. /upload, /practice, homepage" style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Describe the bug *</label>
                    <textarea required name="description" rows={5} placeholder="What happened? What did you expect to happen?"
                      style={{ ...inputStyle, resize: 'vertical' }} />
                  </div>
                  <div>
                    <label style={labelStyle}>Screenshot link (optional)</label>
                    <input type="url" name="screenshotUrl" placeholder="https://imgur.com/..." style={inputStyle} />
                  </div>
