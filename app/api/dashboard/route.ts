...
const NEGATIVE_FLAGS = new Set([
  'too_eager','no_question','generic','needy','boring','low_effort','misread_vibe',
]);
const FLAG_LABELS: Record<string, string> = {
  good_hook:'Strong openers', good_question:'Good questions', witty:'Witty replies',
  specific:'Specific details', good_follow_up:'Follow-up quality',
  showed_personality:'Shows personality', deep_question:'Goes deep',
  matched_energy:'Matches energy', high_effort:'High effort',
  recovered_well:'Recovers from misses', too_eager:'Coming on too strong',
  no_question:'Not asking questions', generic:'Generic messages',
  needy:'Neediness detected', boring:'Low-energy replies',
  low_effort:'Low effort', misread_vibe:'Misreading the vibe',
};
...
function buildSkillProfile(sessions: any[]) {
  ...
  sorted.forEach((session, si) => {
    const msgs = (session.messages ?? []).filter(
      (m: any) => m.role === 'user' && m.analysis?.score != null
    );
    msgs.forEach((m: any) => {
  ...
  const strengths = Object.entries(flagCounts)
    .filter(([f]) => POSITIVE_FLAGS.has(f))
    .sort((a, b) => b[1] - a[1])
    .slice(0, 4)
    .map(([flag, count]) => ({ flag, label: FLAG_LABELS[flag] ?? flag, count }));

  const weaknesses = Object.entries(flagCounts)
    .filter(([f]) => NEGATIVE_FLAGS.has(f))
    .sort((a, b) => b[1] - a[1])
    .slice(0, 4)
    .map(([flag, count]) => ({ flag, label: FLAG_LABELS[flag] ?? flag, count }));
  ...
  const improvement = earlyAvg != null && recentAvg != null
    ? Math.round((recentAvg - earlyAvg) * 10) / 10
    : null;

  const charCounts: Record<string, number> = {};
  sessions.forEach(s => {
    if (s.characterType) charCounts[s.characterType] = (charCounts[s.characterType] ?? 0) + 1;
  });
  const topChar = Object.entries(charCounts).sort((a, b) => b[1] - a[1])[0]?.[0] ?? null;

  const diffCounts = { easy: 0, normal: 0, hard: 0 };
  sessions.forEach(s => {
    if (s.difficulty in diffCounts) diffCounts[s.difficulty as keyof typeof diffCounts]++;
  });

  return {
    avgPracticeScore: allScores.length
    ...
    totalScoredMessages: allScores.length,
    difficultyBreakdown: diffCounts,
    scoreTrendPractice: scoreTrend(allScores),
    hasEnoughData: allScores.length >= 5,
  };
}
...
export async function GET(_request: NextRequest) {
  try {
    // Always require real session — never trust query params
    ...
    const isPremium =
      dbUser.subscriptionStatus === 'paid' ||
      dbUser.subscriptionStatus === 'lifetime';

    // Parallel queries — practice only for premium
    const [allAnalyses, allSessions] = await Promise.all([
      ChatAnalysis.find({ userId })
        .sort({ createdAt: -1 })
        .limit(100)
        .select('conversationScore interestLevel attractionProbability conversationMomentum createdAt missedOpportunities')
        .lean(),
      isPremium
        ? PracticeSession.find({ userId })
            .sort({ createdAt: -1 })
            .limit(100)
            .select('characterType difficulty messageCount currentInterest createdAt messages')
            .lean()
        : Promise.resolve([]),
    ]);
    ...
    const avgScore = analysisScores.length
      ? Math.round((analysisScores.reduce((s: number, v: number) => s + v, 0) / analysisScores.length) * 10) / 10
      : 0;

    const totalPoints = allAnalyses.reduce(
      (sum: number, a: any) => sum + Math.min(Math.round((a.conversationScore ?? 0) * 10), 100),
      0
    );

    const skillInfo = getSkillLevel(totalPoints);
    const streak = calcStreak(allAnalyses.map((a: any) => a.createdAt));
    const trend = scoreTrend(analysisScores);
    const bestScore = analysisScores.length ? Math.max(...analysisScores) : null;
    const avgInterest = allAnalyses.length
      ? Math.round(allAnalyses.reduce((s: number, a: any) => s + (a.interestLevel ?? 0), 0) / allAnalyses.length)
      : 0;
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        practiceCount: isPremium ? allSessions.length : null,
      ...
      skillProfile: isPremium ? buildSkillProfile(allSessions) : null,
      recentAnalyses: allAnalyses.slice(0, 10).map((a: any) => ({
        _id: a._id,
        conversationScore: a.conversationScore,
        interestLevel: a.interestLevel,
        attractionProbability: a.attractionProbability ?? null,
        conversationMomentum: a.conversationMomentum,
        missedOpportunities: Array.isArray(a.missedOpportunities) ? a.missedOpportunities.length : 0,
        createdAt: a.createdAt,
      })),
      practiceSessions: isPremium
        ? allSessions.slice(0, 10).map((p: any) => ({
            _id: p._id,
            characterType: p.characterType,
            difficulty: p.difficulty,
            messageCount: p.messageCount ?? 0,
            currentInterest: p.currentInterest ?? 0,
      ...
      scoreHistory: analysisScores.map((score: number, i: number) => ({
        index: i,
        score,
        date: ([...allAnalyses].reverse()[i] as any)?.createdAt,
      })),
    });
  } catch (error) {
